...
const Modal = (props) => {
  ...
  return show ? (
    <main
      ...
      className="w-full h-full  mx-auto p-6 fixed bg-black bg-opacity-40"
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
        }}
        className="mt-7 p-2 bg-white  rounded-xl shadow-lg dark:bg-gray-800 dark:border-gray-700 h-[85%] w-[40%] fixed top-5 left-[50%] translate-x-[-50%]"
      >
        <div className="w-full text-center flex flex-row items-center justify-center h-14 ">
          <div className="flex-1 text-left text-2xl p-2">
            <i
